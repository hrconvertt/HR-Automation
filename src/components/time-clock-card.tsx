  ...
  if (!hasClockIn) {
    return (
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 rounded-xl bg-slate-50 border border-slate-100">
        ...
        <Link
          href="/dashboard/time"
          className="inline-flex items-center gap-1.5 rounded-lg bg-slate-700 hover:bg-slate-700 text-white px-4 py-2 text-sm font-semibold transition-colors"
        >
          Clock In
    ...
    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 rounded-xl bg-slate-50 border border-slate-100">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          {error && <p className="text-xs text-slate-700 mt-1">{error}</p>}
        </div>
      </div>
      {isClockedIn && (
        <button
          ...
          disabled={busy}
          className="inline-flex items-center gap-1.5 rounded-lg bg-slate-700 hover:bg-slate-700 disabled:opacity-60 disabled:cursor-not-allowed text-white px-4 py-2 text-sm font-semibold transition-colors"
        >
