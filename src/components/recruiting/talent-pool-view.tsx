...
function freshness(updatedAt: string): { label: string; tone: string } {
  ...
  if (days < 30)  return { label: 'Hot',  tone: 'bg-slate-50 text-slate-700 border-slate-100' }
  if (days < 180) return { label: 'Warm', tone: 'bg-slate-50 text-slate-700 border-slate-100' }
...
export function TalentPoolView({ candidates, openRequisitions }: Props) {
  ...
  return (
    <Card className="rounded-xl border-slate-200 overflow-hidden shadow-sm">
      <div className="px-4 py-3 border-b border-slate-100 bg-gradient-to-r from-slate-50/40 to-transparent">
      ...
      <div className="p-4 bg-slate-50/50">
        {filtered.length === 0 ? (
          <div className="text-center py-10">
            <div className="w-12 h-12 rounded-full bg-slate-50 text-slate-300 flex items-center justify-center mx-auto mb-2">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {filtered.map((c) => {
              ...
              return (
                <div key={c.id} className="bg-white border border-slate-200 rounded-xl p-4 hover:border-slate-200 hover:shadow-md transition-all relative group">
                  <button
                    type="button"
                    onClick={() => removeFromPool(c.id)}
                    disabled={removingId === c.id}
                    className="absolute top-2 right-2 p-1 rounded text-slate-300 hover:text-slate-700 hover:bg-slate-50 opacity-0 group-hover:opacity-100 transition"
      ...
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <Send className="w-5 h-5 text-slate-700" /> Invite to Active Role
          </DialogTitle>
        </DialogHeader>
        {target && (
        ...
        <DialogFooter>
          ...
          <Button onClick={submit} disabled={saving || !reqId} className="bg-slate-700 hover:bg-slate-700">
