...
export function InterviewFeedbackButton({
  ...
  const buttonTone =
    initialResult === 'PASS' ? 'text-slate-700 border-slate-100 bg-slate-50' :
    initialResult === 'FAIL' ? 'text-slate-700 border-slate-100 bg-slate-50' :
    initialResult === 'HOLD' ? 'text-slate-700 border-slate-100 bg-slate-50' :
          ...
          <div className="space-y-4 text-sm">
              ...
              <div className="flex gap-1">
                {[1, 2, 3, 4, 5].map((v) => (
                  <button
                    ...
                    type="button"
                    onClick={() => setRating(v)}
                    className={`p-1.5 rounded ${rating >= v ? 'text-slate-500' : 'text-slate-300'} hover:text-slate-300`}
                  >
                    <Star className={`w-5 h-5 ${rating >= v ? 'fill-slate-300' : ''}`} />
                  </button>
                ))}
                {rating > 0 && (
            ...
            <div>
              ...
              {result === 'PASS' && type !== 'ONSITE' && (
                <p className="text-[11px] text-slate-700 mt-2 bg-slate-50 border border-slate-100 rounded px-2.5 py-1.5">
...
function ResultPill({ icon, label, tone, active, onClick }: {
  ...
  const ACTIVE: Record<string, string> = {
    emerald: 'bg-slate-700 text-white border-slate-700',
    rose:    'bg-slate-700 text-white border-slate-700',
    amber:   'bg-slate-500 text-white border-slate-500',
  }
  const IDLE: Record<string, string> = {
    emerald: 'text-slate-700 border-slate-100 hover:bg-slate-50',
    rose:    'text-slate-700 border-slate-100 hover:bg-slate-50',
    amber:   'text-slate-700 border-slate-100 hover:bg-slate-50',
  }
  return (
    <button
      type="button"
