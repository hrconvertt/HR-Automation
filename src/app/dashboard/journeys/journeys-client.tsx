    ...
    <Card>
      ...
      >
        ...
        <div className="hidden sm:block w-48">
          ...
          {blockedCount > 0 && (
            <p className="text-[10px] text-amber-700 mt-1 text-right">{blockedCount} blocking task{blockedCount > 1 ? 's' : ''} open</p>
          )}
        </div>
              ...
              <div key={p.key}>
                <div className="flex items-center gap-2 mb-2">
                  <div className={
                    ...
                    (phaseDone ? 'bg-emerald-500 text-white' : 'bg-slate-200 text-slate-600')
                  }>
    ...
    <div className={
      ...
      <button
        ...
        className={
          ...
          (done ? 'bg-emerald-500 border-emerald-500 text-white' :
           canAct ? 'border-slate-300 hover:border-blue-500 cursor-pointer' :
      ...
      <div className="flex-1 min-w-0">
        <p className={'text-sm ' + (done ? 'line-through text-slate-500' : 'text-slate-900')}>
          {task.title}
          {task.blocking && !done && <span className="ml-2 text-[10px] bg-amber-100 text-amber-800 px-1.5 py-0.5 rounded font-semibold uppercase tracking-wider">Blocking</span>}
        </p>
        {task.description && (
          ...
          <div className="flex items-center gap-3 mt-1.5">
            {generateType && (
              <button
                ...
                className="inline-flex items-center gap-1 text-[11px] text-blue-700 bg-blue-50 hover:bg-blue-100 rounded px-2 py-0.5 font-medium"
              >
      ...
      <div className="flex items-center gap-2 shrink-0 text-xs text-slate-500">
        ...
        {task.dueDate && (
          <span className={isOverdue ? 'text-rose-700 font-semibold' : ''}>
