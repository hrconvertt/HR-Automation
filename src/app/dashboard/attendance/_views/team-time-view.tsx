...
export default function TeamTimeView({ managerEmployeeId, managerName }: { managerEmployeeId: string; managerName: string }) {
  ...
  return (
    <div className="space-y-5">
      ...
      {actionMsg && (
        <div className="text-xs text-slate-900 bg-slate-50 border border-slate-100 rounded-md px-3 py-1.5">
          {actionMsg}
        </div>
      )}
              ...
              <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {(['ONSITE', 'WFH'] as const).map((t) => (
                      <button
                        ...
                        className={
                          ...
                          (workType === t ? 'bg-white text-slate-700' : 'text-white/90 hover:bg-white/10')
                        }
                      >
                ...
                <Button
                  ...
                  className={
                    ...
                    (isIn ? 'bg-white text-slate-700 hover:bg-slate-50' : 'bg-white text-slate-700 hover:bg-slate-50')
                  }
                >
                  {isIn
        ...
        <Card>
          <CardContent className="p-0">
            <div className="px-5 py-3 border-b border-slate-100 flex items-center gap-2 bg-slate-50/50">
              ...
              <p className="text-[11px] uppercase tracking-[0.2em] text-slate-900 font-semibold">Needs Your Attention</p>
              <span className="text-xs font-bold text-slate-700 bg-slate-100 rounded-full px-2 py-0.5">{exceptions.length}</span>
            </div>
            <ul>
              {exceptions.map((ex) => (
                <li key={ex.id} className="flex items-center justify-between px-5 py-3 border-b border-slate-50 last:border-b-0">
                  ...
                  <Link href={`/dashboard/employees/${ex.record.employeeId}`} className="text-xs text-slate-700 hover:underline">View</Link>
                </li>
              ))}
            </ul>
          </CardContent>
        ...
        <div className="flex items-baseline justify-between flex-wrap gap-2 mb-3">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">
              <span className="text-slate-700 font-semibold">{team.filter((r) => r.isCurrentlyIn).length}</span> working now
              {onLeave.length > 0 && <> · <span className="text-slate-700 font-semibold">{onLeave.length}</span> on leave</>}
            </p>
          </div>
...
function TeamCard({ record, now }: { record: TodayRecord; now: Date }) {
  ...
  const tone = isIn      ? { bg: 'bg-slate-50',  ring: 'ring-slate-100',  avatar: 'bg-slate-500',  pill: 'bg-slate-100 text-slate-900', dotClr: 'bg-slate-500 animate-pulse' }
             : isDone    ? { bg: 'bg-slate-50',    ring: 'ring-slate-200',    avatar: 'bg-slate-400',    pill: 'bg-slate-200 text-slate-700',     dotClr: 'bg-slate-400' }
             : isLeave   ? { bg: 'bg-slate-50',     ring: 'ring-slate-100',     avatar: 'bg-slate-500',     pill: 'bg-slate-100 text-slate-900',       dotClr: 'bg-slate-500' }
             :             { bg: 'bg-slate-50/60', ring: 'ring-slate-100',    avatar: 'bg-slate-300',    pill: 'bg-slate-100 text-slate-900',     dotClr: 'bg-slate-500' }
      ...
      <div className="p-4">
        ...
        <div className="mt-3 flex items-baseline justify-between">
          {isIn || isDone ? (
            ...
            <p className="text-xs text-slate-700">Approved leave</p>
          ) : (
            <p className="text-xs text-slate-700">Haven&apos;t clocked in</p>
          )}
        </div>
