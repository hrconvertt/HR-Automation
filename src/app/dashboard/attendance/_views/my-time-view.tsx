        ...
        <CardContent className="p-0">
          ...
          <div className="px-6 py-2.5 border-b border-slate-200 bg-slate-50/50 flex items-center justify-between">
            ...
            <span className={
              ...
              <span className={
                ...
                (isClockedIn ? 'bg-slate-500 animate-pulse' :
                 isDoneForDay ? 'bg-slate-400' : 'bg-slate-500')
              } />
              ...
              {(() => {
                ...
                return (
                  <p className="text-sm text-slate-500 mt-2">
                    ...
                    {isClockedIn && (
                      <> · this session <strong className="text-slate-700 tabular-nums">{formatHMS(currentSessionMs)}</strong></>
                    )}
                  ...
                  <button
                    ...
                    className={
                      ...
                      ((isClockedIn ? isWfh : workType === 'WFH') === (t === 'WFH')
                        ? (t === 'WFH' ? 'bg-slate-700 text-white shadow' : 'bg-slate-700 text-white shadow')
          ...
          {actionMsg && (
            <div className={
              ...
              (actionMsg.tone === 'ok' ? 'bg-slate-50 border-slate-100 text-slate-900' :
               actionMsg.tone === 'warn' ? 'bg-slate-50 border-slate-100 text-slate-900' :
      ...
      <Card>
        <CardContent className="p-5">
              ...
              return (
                <div
                  ...
                  className={
                    ...
                    (isToday
                      ? (isWfh ? 'border-slate-300 bg-slate-50 ring-1 ring-slate-100' : 'border-slate-300 bg-slate-50 ring-1 ring-slate-100')
                      : isFuture
                ...
                >
                  ...
                  {isFuture ? (
                    ...
                    <>
                      <p className={'text-xs font-semibold tabular-nums mt-1 ' + (isWfh ? 'text-slate-700' : 'text-slate-700')}>{hours.toFixed(1)}h</p>
                      <p className={'text-[9px] uppercase tracking-wider font-semibold mt-0.5 ' + (isWfh ? 'text-slate-500' : 'text-slate-500')}>
                        {isWfh ? '🏠 WFH' : '🏢 Onsite'}
                      </p>
                    </>
                  ) : isWeekend ? (
                    ...
                    <p className="text-[10px] text-slate-700 mt-1 font-semibold">Leave</p>
                  ) : (
                    <p className="text-[10px] text-slate-400 mt-1">—</p>
                  )}
                </div>
          ...
          <div className="flex items-center gap-3 mt-3 text-[10px] text-slate-500">
            <span className="flex items-center gap-1"><span className="w-2.5 h-2.5 rounded-sm bg-slate-100" /> Onsite</span>
            <span className="flex items-center gap-1"><span className="w-2.5 h-2.5 rounded-sm bg-slate-100" /> WFH</span>
            <span className="flex items-center gap-1"><span className="w-2.5 h-2.5 rounded-sm bg-slate-100" /> Leave</span>
