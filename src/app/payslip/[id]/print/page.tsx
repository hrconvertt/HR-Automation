...
export default async function PrintPayslipPage({ params }: PageProps) {
  ...
  // The two stacks, as data. Blanks are the issued slip's own spacing and are
  // what holds Gross Salary level with Other Deductions.
  ...
  const payLabels = [
    ...
    'Over Time/Bonus', 'Arrears', 'Other Allowances', '', 'Monthly Allowance',
  ]
  const payValues = [
  ...
  const dedLabels = [
    'Income tax', 'EOBI', 'Health care', 'Deduction (Loan /', 'Monthly Vehicle)',
    'Advance Deduction', '', '', 'Other Deductions',
  ]
  const dedValues = [
    dashed(ded.incomeTax), dashed(ded.eobi), dashed(ded.healthcare), '',
    dashed(ded.loanAndVehicle), dashed(ded.advance), '', '-', dashed(ded.other),
  ]
  ...
  return (
    <>
      ...
      >
        ...
        {/* Letterhead: everything ranged left under the mark, small. There
            is no right-hand address block on the issued slip. */}
        <div style={{ marginBottom: 10 }}>
          <img
            ...
            alt="Convertt"
            style={{ height: 22, display: 'block', marginBottom: 4 }}
          />
          <div style={{ fontSize: 7.5, lineHeight: 1.45, color: '#000' }}>
        ...
        {/* One bordered grid from the title through to Net Pay. All four
            columns are shared by every section, which is what keeps the
            Deductions divider running straight down the page.

            The pay and deduction bodies are single tall cells rather than a
            row each. The two sides hold different numbers of entries — five
            deductions against three allowances before Gross Salary — and the
            issued slip rules no line between them, so a row-per-entry grid
            could not reproduce it however the rows were padded. */}
        <table style={{
          ...
          <tbody>
            <tr>
              <td colSpan={4} style={{
                ...cell, background: '#dce6f1', textAlign: 'center', fontWeight: 700,
            ...
            {/* Employee details — two tall cells, not a five-row grid. */}
            <tr>
              <td colSpan={2} style={{ ...cell, lineHeight: 1.75 }}>
                <L k="Employee Number" v={payslip.employee.employeeCode} />
                <div style={{ height: '1.75em' }} />
                <L k="DOJ" v={fmtDDMMYYYY(payslip.employee.joiningDate)} />
            ...
            {leaveRows.map((r) => (
              <tr key={r.label}>
                <td style={{ ...cell, fontWeight: 700, ...noRule }}>{r.label}</td>
                <td style={{ ...cell, textAlign: 'center', ...noRule }}>{r.bal.allocated || '-'}</td>
                <td style={{ ...cell, textAlign: 'center', ...noRule }}>{r.availed || '-'}</td>
                <td style={{ ...cell, textAlign: 'center', ...noRule }}>{r.bal.remaining || '-'}</td>
              </tr>
            ))}

            <tr>
              <td style={{ ...cell, fontWeight: 700, borderTop: '1px solid #000' }}>Pay &amp; Allowances</td>
              <td style={{ ...cell, fontWeight: 700, textAlign: 'center', borderTop: '1px solid #000' }}>Rs.</td>
              <td style={{ ...cell, fontWeight: 700, borderTop: '1px solid #000' }}>Deductions</td>
              <td style={{ ...cell, fontWeight: 700, textAlign: 'center', borderTop: '1px solid #000' }}>Rs.</td>
            </tr>
            ...
            <tr>
              ...
              <td style={{ ...cell, textAlign: 'center', lineHeight: 1.9 }}>
                {payValues.map((v, i) => (
...
const cell: React.CSSProperties = {
  border: '1px solid #000', padding: '2px 5px', fontSize: 9,
  verticalAlign: 'top', color: '#000',
}

/** No horizontal rule between leave rows — the issued slip has none. */
const noRule: React.CSSProperties = { borderTop: 'none', borderBottom: 'none' }

/** "Employee Number: CON-UIUX-040" — label bold, value not. */
