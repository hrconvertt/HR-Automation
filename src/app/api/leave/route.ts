 ...
 *   - Mon → Wed                     → 3
 *   - Mon → Fri (Wed is holiday)    → 4
 *   - Fri only                      → 1
 *   - Fri → Mon  (sandwich)         → 4  (Fri + Sat + Sun + Mon)
 *   - Mon → Tue, firstDayHalf=true  → 1.5
 *   - Mon → Mon, firstDayHalf=true  → 0.5  (single-day half)
 */
...
// (was: isoKey — now provided by `dayKey` in @/lib/date-utils)
...
export async function GET(request: NextRequest) {
  ...
  // ── For PENDING + PENDING_HR requests, attach the requester's current
  //    balance for the leave type they're asking for — so approvers at
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // ── Authorisation: who is this leave FOR? ────────────────────────────
    ...
    // ── Overlap check: any existing pending/approved request for the same
    //    employee that intersects this date range → refuse.
    ...
    if (overlap) {
      const range = `${overlap.fromDate.toLocaleDateString('en-GB', { day: 'numeric', month: 'short' })} – ${overlap.toDate.toLocaleDateString('en-GB', { day: 'numeric', month: 'short' })}`
      return NextResponse.json({
    ...
    // ── Pull public holidays in the range — those days are free ───────────
    ...
    // Notify the direct manager + all HR users — best-effort, parallel.
