...
<body>
  ...
  <script>
    // Auto-fire the print dialog once the page renders so the user can
    // immediately "Save as PDF" — same pattern as the increment-letter print page.
  ...
  return new NextResponse(html, {
    status: 200,
    headers: {
      // Inline so the browser renders it in the new tab (downloading the raw
      // HTML file via Content-Disposition: attachment is what previously
      // broke the Total Rewards button — Chrome would save a useless .html
      // instead of opening a printable page).
      'Content-Type': 'text/html; charset=utf-8',
      'Content-Disposition': `inline; filename="total-rewards-${target.employeeCode}-${currentYear}.html"`,
